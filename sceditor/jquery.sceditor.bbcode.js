(function ($) {
    ...
    function removeWhiteSpace(root) {
        var nodeValue,
            ...
            nextNode,
            trimStart,
            cssWhiteSpace = css(root, 'whiteSpace'), // Preserve newlines if is pre-line
    ...
    var defaultOptions = {
         ...
         * @type {string}
         */
        fonts: '宋体,黑体,Arial,Arial Black,Book Antiqua,Century Gothic,Comic Sans MS,Courier New,' + 'Georgia,Serif,Impact,Tahoma,Times New Roman,Trebuchet MS,Script MT Bold,Stencil,Verdana,Lucida Console',
         ...
         * @type {string}
         */
        colors: 'skyblue,royalblue,blue,darkblue|' + 'orange,orangered,crimson,red|' + 'firebrick,darkred,green,limegreen|' + 'seagreen,teal,deeppink,tomato|' + 'coral,purple,indigo,burlywood|' + 'sandybrown,sienna,chocolate,silver',
    ...
    function uriScheme(url) {
        var path, // If there is a : before a / then it has a scheme
            hasScheme = /^[^\/]*:/i,
    ...
    var _templates = {
        ...
        fontOpt: '<a class="sceditor-font-option" href="#" ' + 'data-font="{fontname}"><font face="{font}">{font}</font></a>',

        sizeOpt: '<a class="sceditor-fontsize-option" data-size="{size}" ' + 'href="#"><span style="font-size:{size}">{size}</span></a>',
    ...
    var defaultCmds = {
        ...
        font: {
            _dropDown: function (editor, caller, callback) {
                ...
                editor.opts.fonts.split(',').forEach(function (font) {
                    ...
                    var fontname = font;
                    if (font === '宋体') fontname = 'simsun';
                    if (font === '黑体') fontname = 'simhei';
        ...
        size: {
            ...
            exec: function (caller) {
                ...
                defaultCmds.size._dropDown(editor, caller, function (fontSize) {
                    editor.wysiwygEditorInsertHtml('<span style="font-size:' + fontSize + '">', '</span>');
        ...
        indent: {
            state: function (parent, firstBlock) {
                ...
                if (is(firstBlock, 'ul,ol,menu')) {
                    ...
                    if (startParent !== startParent.parentNode.firstElementChild || // work around a bug in FF
                        (is(endParent, 'li') && endParent !== endParent.parentNode.lastElementChild)) {
                        return 0;
                    }
                }
        ...
        image: {
            _dropDown: function (editor, caller, selected, cb) {
                ...
                on(content, 'click', '.button', function (e) {
                    if (urlInput.value) {
                        cb(urlInput.value, find(content, '#width')[0].value, find(content, '#height')[0].value);
                    }
        ...
        at: {
            _dropDown: function (editor, caller, cb) {
                ...
                content.innerHTML = '<div><label for="uinfo">用户名或UID</label> ' + '<input type="text" id="uinfo" dir="ltr"  /></div>' + '<div><input type="button" class="button" value="确定" /></div>';

                var tmpInput = find(content, '#uinfo')[0];
                ...
                on(content, 'click', '.button', insertat);
                on(content, 'keypress', function (e) {
                    // 13 = enter key
                    if (e.which === 13 && tmpInput.value) {
                        insertat(e);
                    }
                }, EVENT_CAPTURE);

                editor.createDropDown(caller, 'insertat', content);
            },
            exec: function (caller) {
                var editor = this;
                defaultCmds.at._dropDown(editor, caller, function (uinfo) {
                    editor.insertText('[@' + uinfo + ']');
                });
            },
        ...
        gotid: {
            _dropDown: function (editor, caller, cb) {
                ...
                content.innerHTML = '<div><label for="tid">主题ID</label> ' + '<input type="text" id="tid" dir="ltr"  /></div>' + '<div><label for="desc">显示文字(选填)</label> ' + '<input type="text" id="desc" dir="ltr"  /></div>' + '<div><input type="button" class="button" value="确定" /></div>';

                var tmpInput = find(content, '#tid')[0];

                function inserttid(e) {
                    if (tmpInput.value) {
                        cb(tmpInput.value, find(content, '#desc')[0].value);
                    }
                ...
                on(content, 'click', '.button', inserttid);
                on(content, 'keypress', function (e) {
                    // 13 = enter key
                    if (e.which === 13 && tmpInput.value) {
                        inserttid(e);
                ...
                editor.createDropDown(caller, 'inserttid', content);
            },
            exec: function (caller) {
                var editor = this;
                defaultCmds.gotid._dropDown(editor, caller, function (tid, desc) {
                    if (!desc && !editor.getRangeHelper().selectedHtml()) {
                        editor.insertText('[tid]' + tid + '[/tid]');
                    } else {
                        if (desc) {
                            editor.insertText('[tid=' + tid + ']' + desc + '[/tid]');
                        } else {
                            editor.insertText('[tid=' + tid + ']' + editor.getRangeHelper().selectedHtml() + '[/tid]');
                        }
                    }
                });
            },
        ...
        gopid: {
            _dropDown: function (editor, caller, cb) {
                ...
                content.innerHTML = '<div><label for="pid">回复ID</label> ' + '<input type="text" id="pid" dir="ltr"  /></div>' + '<div><label for="desc">显示文字(选填)</label> ' + '<input type="text" id="desc" dir="ltr"  /></div>' + '<div><input type="button" class="button" value="确定" /></div>';

                var tmpInput = find(content, '#pid')[0];

                function insertpid(e) {
                    if (tmpInput.value) {
                        cb(tmpInput.value, find(content, '#desc')[0].value);
                    }
                ...
                on(content, 'click', '.button', insertpid);
                on(content, 'keypress', function (e) {
                    // 13 = enter key
                    if (e.which === 13 && tmpInput.value) {
                        insertpid(e);
                ...
                editor.createDropDown(caller, 'insertpid', content);
            },
            exec: function (caller) {
                var editor = this;
                defaultCmds.gopid._dropDown(editor, caller, function (pid, desc) {
                    if (!desc && !editor.getRangeHelper().selectedHtml()) {
                        editor.insertText('[pid]' + pid + '[/pid]');
                    } else {
                        if (desc) {
                            editor.insertText('[pid=' + pid + ']' + desc + '[/pid]');
                        } else {
                            editor.insertText('[pid=' + pid + ']' + editor.getRangeHelper().selectedHtml() + '[/pid]');
                        }
                    }
                });
            },
        ...
        dice: {
            _dropDown: function (editor, caller, cb) {
                ...
                content.innerHTML = '<div><label for="dice1">骰子面数</label> ' + '<input type="number" id="dice1" dir="ltr"  /></div>' + '<div><label for="dice2">骰子个数</label> ' + '<input type="number" id="dice2" dir="ltr" value="1" /></div>' + '<div><input type="button" class="button" value="确定" /></div>';

                var tmpInput = find(content, '#dice1')[0];
                var tmpInput2 = find(content, '#dice2')[0];

                function insertdice(e) {
                    if (tmpInput.value && tmpInput2.value) {
                        cb(tmpInput.value, tmpInput2.value);
                    }
                    editor.closeDropDown(true);
                    e.preventDefault();
                }

                on(content, 'click', '.button', insertdice);
                editor.createDropDown(caller, 'insertdice', content);
            },
            exec: function (caller) {
                var editor = this;
                defaultCmds.dice._dropDown(editor, caller, function (dice1, dice2) {
                    if (dice2 == 1) {
                        editor.insertText('[dice]d' + dice1 + '[/dice]');
                    } else if (dice2 > 1) {
                        editor.insertText('[dice]' + dice2 + 'd' + dice1 + '[/dice]');
                    }
                });
            },
            txtExec: function (caller, selected) {
        ...
        collapse: {
            _dropDown: function (editor, caller, cb) {
                ...
                content.innerHTML = '<div><label for="desc">提要(选填)</label> ' + '<input type="text" id="desc" dir="ltr"  /></div>' + '<div><label for="text">请输入要折叠的内容</label> ' + '<textarea cols="50" rows="15" id="text"></textarea></div>' + '<div><input type="button" class="button" value="确定" /></div>';

                var tmpInput = find(content, '#text')[0];

                function insertcollapse(e) {
                    if (tmpInput.value) {
                        cb(tmpInput.value, find(content, '#desc')[0].value);
                    }
                ...
                on(content, 'click', '.button', insertcollapse);

                editor.createDropDown(caller, 'insertcollapse', content);
            },
            exec: function (caller) {
                var editor = this;
            ...
            txtExec: function (caller, selected) {
                ...
                defaultCmds.collapse._dropDown(editor, caller, function (text, desc) {
                    if (!desc) {
                        ...
                        editor.insertText('[collapse=' + desc + ']' + text + '[/collapse]');
                    }
                });
            },
            tooltip: '插入折叠的内容',
            ...
            exec: function (caller) {
                ...
                var createContent = function (includeMore) {
                    var moreLink,
                        ...
                        perLine = 0,
                        emoticons = extend({}, opts.emoticons.dropdown, includeMore ? opts.emoticons.more : {});
        ...
        base.replaceKeyword = function (keywords, includeAfter, keywordsSorted, longestKeyword, requireWhitespace, keypressChar) {
            ...
            while (keywordIdx--) {
                ...
                if (requireWhitespace) {
                    match = outerText
                        .substr(startIndex)
                        .match(new RegExp(whitespaceRegex + regex(keyword) + whitespaceRegex));
        ...
        initEmoticons = function () {
            ...
            each(allEmoticons, function (key, url) {
                allEmoticons[key] = _tmpl('emoticon', {
                    key: key, // Prefix emoticon root to emoticon urls
     ...
     */
    SCEditor.command = /** @lends sceditor.command */
        {
            /**
             * Gets a command
             *
